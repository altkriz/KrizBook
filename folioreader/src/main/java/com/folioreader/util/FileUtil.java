...
public class FileUtil {
    ...
    public static String saveEpubFileAndLoadLazyBook(final Activity activity,
        ...
        try {
            ...
            if (!isFolderAvailable) {
                if (epubSourceType.equals(FolioActivity.EpubSourceType.RAW)) {
                    epubInputStream = activity.getResources().openRawResource(epubRawId);
    ...
    public static String getFolioEpubFolderPath(String epubFileName) {
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.P) {
            return Functions.getAppFolder(mActivity) + "/" + FOLIO_READER_ROOT + "/" + epubFileName;
        } else {
